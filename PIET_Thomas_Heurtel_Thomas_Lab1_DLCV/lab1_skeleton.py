...
def computeEntropy(img):
   #source :  stackoverflow.com/questions/50313114/what-is-the-entropy-of-an-image-and-how-is-it-calculated
    glcm = np.squeeze(greycomatrix(img, distances=[1], angles=[0], symmetric = True, normed = True))
    ent = -np.sum(glcm*np.log2(glcm+(glcm==0)))
    return ent
...
if __name__=='__main__':
    ...
    while(cap.isOpened()):
        ...
        if (len(previousFrames) >= deltaT):
           ...
           #test
           #curr = np.ones((w,h))*255   
           #prev = np.zeros((w,h))
           #mse = computeMSE(curr,prev)
           #if(mse!=0):
           #    return "error"
